"""Batch API with explicit simulated and live modes."""
...
app = FastAPI(title="Day26 offline batch API", debug=False)
...
def make_response(rows: list[TaskReport], mode: Literal["simulated", "live"] = "simulated") -> dict:
    retry_result_list = []
    task_list = []
    for row in rows:
    ...
    result = {"mode": mode, "tasks": task_list, "summary": summary}
    return result
...
def analyze_batch(
    ...
    return make_response(rows, mode)
