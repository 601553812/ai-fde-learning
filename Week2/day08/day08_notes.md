...
2. 为什么 CLI 测试要替换 `Week2.day08.cli.fetch_requirement`，而不是继续替换 `requests.get`？
...
```python
monkeypatch.setattr("Week2.day08.test_day08.FakeResponse.json", json_error)
...
- 第 2 题：“不能多层嵌套测试”不是硬性规则。测试可以调用多层真实代码；本题为聚焦 `run()` 的参数、写文件和错误处理，替换直接依赖 `Week2.day08.cli.fetch_requirement`，HTTP 客户端本身另有测试。
