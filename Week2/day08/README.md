...
```text
test_day08
  → Week2.day08.cli.run(argv)
      → Week2.day08.cli.fetch_requirement(url, timeout_seconds)
...
```powershell
.\.venv\Scripts\python.exe -m Week2.day08.cli <URL> <OUTPUT> --timeout 2.5
```
...
测试 `Week2.day08.cli.run()` 时，被测对象是 `run()`；它的外部依赖是 `fetch_requirement()`。这里不需要再深入模拟 `requests.get()`，否则一个 CLI 测试会同时验证太多层。
...
所以 `run()` 执行时查找的是 `Week2.day08.cli.fetch_requirement`。CLI 测试应替换这个名称：

```python
monkeypatch.setattr("Week2.day08.cli.fetch_requirement", fake_fetch)
```
...
修改文件：`Week2/day08/test_day08.py`
...
3. Arrange：替换 `Week2.day08.cli.fetch_requirement`，用 `tmp_path / "output.json"` 作为输出路径。
...
```powershell
.\.venv\Scripts\python.exe -m pytest .\Week1\day06\test_day06.py .\Week1\day07\test_day07.py .\Week2\day08\test_day08.py -q
```
...
```powershell
.\.venv\Scripts\python.exe -m Week2.day08.cli http://127.0.0.1:8000/sample_requirement.json .\Week2\day08\output.json --timeout 2.5
```

测试结束后删除手动生成的 `Week2/day08/output.json`，不要提交生成结果。
...
- `python -m Week2.day08.cli --help` 返回 `0`。
