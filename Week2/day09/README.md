...
需要修改的文件只有 `Week2/day09/app.py` 和 `Week2/day09/test_day09.py`。`models.py`、路由登记和大部分测试已经提供。Day 9 现已归档到 Week2，使用 `Week1.day06` 导入第一周的已有代码。
...
```text
.\.venv\Scripts\python.exe -m pytest Week2/day09/test_day09.py -q --tb=short
.\.venv\Scripts\python.exe -m pytest Week1/day06/test_day06.py Week1/day07/test_day07.py Week2/day08/test_day08.py Week2/day09/test_day09.py -q --tb=short
```
...
脚手架已在 Python 3.11.15、FastAPI 0.141.1、Uvicorn 0.52.4、HTTPX 0.28.1 上验证上述初始结果，`pip check` 通过。当前间接依赖会显示两条弃用警告（Starlette/HTTPX 和 AnyIO），不影响今天的测试；它们不是 TODO 错误，今天无需处理或屏蔽。测试失败先找底部的 `Week2/day09/app.py` 或 `Week2/day09/test_day09.py`，不需要逐层阅读框架调用栈。
...
```text
.\.venv\Scripts\python.exe -m uvicorn Week2.day09.app:app --host 127.0.0.1 --port 8000 --reload
```

`Week2.day09.app:app` 中，冒号前是模块名（`Week2/day09/app.py`），冒号后是该模块的应用对象名。`--reload` 在保存代码后重新加载，仅用于本地开发。端口占用时改为 8001，并同步修改下面 URL。
...
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day09/sample_request.json" http://127.0.0.1:8000/analyze-requirement
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day09/invalid_request.json" http://127.0.0.1:8000/analyze-requirement
```
