...
只需要修改 `Week2/day10/app.py`、`Week2/day10/test_day10.py` 和学习记录。保留路由已提供的调用顺序、模型和原解析规则。
...
- `test_over_limit_precedes_business_check` 已提供 monkeypatch：替换的是路由模块使用的 `Week2.day10.app.parse_requirement`，一旦它被调用测试就失败。这是验证“必须先拒绝再解析”，不是模拟 HTTP 返回结果。
...
```text
.\.venv\Scripts\python.exe -m pytest Week2/day10/test_day10.py -q --tb=short
.\.venv\Scripts\python.exe -m pytest Week1/day06/test_day06.py Week1/day07/test_day07.py Week2/day08/test_day08.py Week2/day09/test_day09.py Week2/day10/test_day10.py -q --tb=short
```
...
```text
.\.venv\Scripts\python.exe -m uvicorn Week2.day10.app:app --host 127.0.0.1 --port 8010 --reload
```
...
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day10/incomplete_request.json" http://127.0.0.1:8010/analyze-requirement
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day10/incomplete_request.json" "http://127.0.0.1:8010/analyze-requirement?strict=true"
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day09/sample_request.json" "http://127.0.0.1:8010/analyze-requirement?strict=true"
curl.exe -i -H "Content-Type: application/json" --data-binary "@Week2/day09/invalid_request.json" http://127.0.0.1:8010/analyze-requirement
```
